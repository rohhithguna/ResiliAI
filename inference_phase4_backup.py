...
rl_used = UsageCounter(0)
rule_used = UsageCounter(0)
...
def select_action(state):
    ...
    action = coordinator.select_action(state)

    # Action 0 is treated as model/no-op path; non-zero actions are explicit rules.
    if action == 0:
        rl_used.inc(1)
    else:
        rule_used.inc(1)
